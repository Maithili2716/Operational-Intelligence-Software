...
function checkMissingApproval(context){
     ...
     if(compliance.approvalStatus === "APPROVED")
          return[]
     return [AttentionItem.createAttentionItem(context,"HIGH","COMPLIANCE",`${context.entityType} ${context.entityId} is missing approval`)];
...
function checkMissingOwner(context){
     ...
     if(compliance.owner)
          return[];     
     return [AttentionItem.createAttentionItem(context,"HIGH","COMPLIANCE",`${context.entityType} ${context.entityId} is missing owner`)];
...
function checkMandatoryFields(context){
     ...
     if(compliance.mandatoryFieldsComplete)
          return[];     
     return [AttentionItem.createAttentionItem(context,"HIGH","COMPLIANCE",`${context.entityType} ${context.entityId} is missing the mandatory fields`)];
